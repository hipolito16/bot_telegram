...
func (self *AdminController) Add(update tgbotapi.Update) {
	if _, ok, user := middlewares.History.HasCommandHistory(update); !ok {
		...
		middlewares.History.AddCommandHistory(update, user)
		msg := bot.NewMessage(update.Message.Chat.ID, "Informe o nome para esse usuário.")
		self.bot.Send(msg)
	} else {
		user.Name = update.Message.Text
		database.DB.Create(&user)
		middlewares.Auth.RefreshUsers()
		middlewares.History.RemoveCommandHistory(update)
